...
# NOTE: keep API key out of source in production; use env var or secret manager
client = openai.OpenAI(api_key=os.getenv('OPENAI_API_KEY'))
OPENAI_KEY = os.getenv('OPENAI_API_KEY')    
if not OPENAI_KEY:
    # Try to prompt the user for a key (hidden) when running interactively
    try:
        if os.isatty(0):
            import getpass
            OPENAI_KEY = getpass.getpass(prompt="Enter your OpenAI API key (input will be hidden): ") or None
    except Exception:
        OPENAI_KEY = None

if not OPENAI_KEY:
    raise RuntimeError("OpenAI API key not found. Set the OPENAI_API_KEY environment variable or run interactively and provide the key.")
...
# Note: Bearer token is now passed dynamically from views.py instead of being hardcoded

def get_peptalk_voice_url(bearer_token=None):
    """Fetch peptalk voice URL from the API using dynamic bearer token"""
    try:
        headers = {}
        if bearer_token:
            headers["Authorization"] = f"Bearer {bearer_token}"
        response = requests.get(PEPTALK_SETTINGS_URL, headers=headers)
...
def get_peptalk_voice_url_by_emotion(emotion_title=None, bearer_token=None):
...
def get_schedule_settings(bearer_token=None):
    """Fetch schedule/task settings from configured API using dynamic bearer token. Returns a list of task dicts or empty list on failure."""
    try:
        headers = {}
        if bearer_token:
            headers["Authorization"] = f"Bearer {bearer_token}"
...
def analyze_mama_emotions(user_input, *, seed=None):
    """Analyze mama's emotions. Prioritizes keyword detection for reliability, with AI as backup."""
    # First, always run keyword detection as it's more reliable
    keyword_emotions = detect_emotions_by_keywords_updated(user_input)
    
    # If keyword detection found something, use it directly
    if keyword_emotions.get('confidence', 0) > 0.5:
        return keyword_emotions
    
    # Otherwise, try AI-based detection as backup
    prompt = (
        "Analyze the following message for emotional indicators and return ONLY a valid JSON object with keys: "
    ...
    try:
        ...
        try:
            ...
            print(f"JSON parse error: {json_error}, falling back to keyword detection")
            return keyword_emotions
    except Exception as e:
        print(f"Emotion analysis error: {e}, using keyword detection")
        return keyword_emotions
...
def query_existing_tasks(user_input, bearer_token=None):
