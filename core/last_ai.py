...
SCHEDULE_BEARER_TOKEN = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJ0b2tlbl90eXBlIjoiYWNjZXNzIiwiZXhwIjoxNzU5NTM1MTgzLCJpYXQiOjE3NTg5MzAzODMsImp0aSI6ImVlMmE2ZDY0YjAyZDRjYjI4YmU0NDI5ZTc3ZGViNDU0IiwidXNlcl9pZCI6M30.8Fk6CFdyCb8wtZai9F2UQ8eydHM42zbHE-PwD-ujB30'
        ...
        if response.status_code == 200:
            ...
            if isinstance(data, dict) and 'cities' in data:
                ...
                if voice_urls:
                    ...
                    # Normalize and format as requested: /media/voices/filename.mp3
                    # Extract filename if the API returns a full path or just a filename
                    # Examples handled:
                    # - "/media/voices/86ea-...mp3"
                    # - "media/voices/86ea-...mp3"
                    # - "86ea-...mp3"
...
def get_schedule_settings():
    ...
    try:
        headers = {
            'Authorization': f'Bearer {SCHEDULE_BEARER_TOKEN}'
        }
        response = requests.get(SCHEDULE_SETTINGS_URL, headers=headers)
        if response.status_code == 200:
            data = response.json()
